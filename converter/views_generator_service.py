...
class ViewsGeneratorService:
    ...
    @staticmethod
    def generate_views(video_meta, center_pins, youtube_metrics):
        """
        ...
        views = {
            "competitive": ViewsGeneratorService._generate_competitive_view(center_pins, youtube_metrics),
            "education": ViewsGeneratorService._generate_education_view(center_pins),
            "self_improvement": ViewsGeneratorService._generate_self_improvement_view(center_pins),
        }

        return views

    @staticmethod
    def _generate_competitive_view(center_pins, youtube_metrics):
        """YouTube メトリクス + top_pins_by_engagement"""

        # Snapshot timestamp（JST）
        now = datetime.now(ViewsGeneratorService.JST).isoformat()
            ...
            "top_pins_by_engagement": [
                {
                    ...
                    "type": item['pin']['type'],
                    "content": ViewsGeneratorService._truncate_content(item['pin']['content']),
    ...
    @staticmethod
    def _generate_education_view(center_pins):
            ...
            top_pins_by_difficulty[difficulty] = [
                {
                    ...
                    "content": ViewsGeneratorService._truncate_content(pin['content']),
    ...
    @staticmethod
    def _generate_self_improvement_view(center_pins):
