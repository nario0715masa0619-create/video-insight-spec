...
【重要な制約】
1. 「このパターンで必ずヒットする」「高い反応を確実に得られる」という表現は禁止。
2. 「このテーマ×段階×形式の組み合わせは、視聴者が学習したいニーズを示唆している（仮説）」という慎重な表現を使用。
3. スコア（0.89など）は「コンテンツの質の高さ」を示すもので、「視聴者反応の強さ」ではないことを明示。
4. 実際のエンゲージメント率が低い場合は「ターゲット層への発見機会が不足している可能性」を指摘。

【出力フォーマット】
- 結論：このテーマ×形式の学習構造が、特定の視聴者ニーズを示唆しているという仮説を述べる
- 意味：そのニーズはどういう背景から生じているのかを解説
- 提案：そのニーズに応えるために実行可能な具体的施策のみ
  例：SEO対策（タイトル・説明欄のキーワード最適化）、プレイリスト作成、SNS投稿（Twitter/LinkedIn）、
      他チャンネルとの相互プロモーション、YouTubeコミュニティ機能での告知、メールニュースレター配信等
  ※「推奨システムを強化」「プロモーション活動」など抽象的・実行不可能な表現は禁止
- 根拠：スコア（品質）とエンゲージメント率を、「質は高いが発見機会が不足」という観点から説明

数字は慎重に扱い、過度な自信を表現しないこと。
"""
