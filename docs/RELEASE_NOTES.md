## v1.5.4 – Free-Trial Dashboard Demo Experience (2026-06-10)

### 変更内容
- **無料1本解析を専用モードで分離**: VIS_MODE 環境変数により通常モード（デモ5講座）と無料1本解析モードが独立して動作するよう分離。
- **対象動画の自動選択機能**: ダッシュボード起動時に target_video が自動で選択されるよう UI を改善。
- **既存機能の完全維持**: pytest 101/101 PASS にて、通常モードが一切影響を受けないことを保証。

- **無料1本解析の標準運用を再定義**: レポート納品から「ダッシュボード実演」へ変更
- **report.md の位置づけ変更**: 補助資料・後送資料として位置づけ直し
- **営業向け運用ガイド整備**: 商談フロー・チェックリスト・実演順序を明文化

### 詳細
- 無料1本解析の価値は「対象動画をVISに通したダッシュボード画面を見せること」に集約
- 最低ラインは以下の3点:
  1. 対象動画をVISに通す
  2. ダッシュボード上で診断・改善提案を実演する
  3. 必要に応じて report.md を後送資料として返す
- 商談時の標準見せ方を明記:
  1. 【品質診断】タブ → 役割・強み・弱み・欠けている橋渡し
  2. 【改善提案】タブ → 最優先・次点アクション
  3. 必要に応じて他タブを補足で見せる

### ドキュメント更新
- `docs/FREE_TRIAL_ONE_VIDEO_OPERATION.md` – ダッシュボード実演中心に改版
- `docs/OPERATION_GUIDE.md` – 無料1本解析（体験デモ）セクション追加
- `docs/PROJECT_STATUS.md` – 位置づけを「デモ体験」に修正
- `docs/FREE_TRIAL_DEMO_CHECKLIST.md` – 営業向け簡易チェックリスト新規作成
- `README.md` – 無料1本解析の説明を修正
- `scripts/run_free_trial_one_video.py` – ダッシュボード起動案内を追加

### 実装影響
- ダッシュボード機能に変更なし
- スクリプト機能に変更なし
- pytest 101/101 PASS（ベースライン維持）

### 運用上の意味
- VIS の価値を「AIレポート」ではなく「対象動画の実況解析体験」として営業に周知
- ダッシュボード実演により、見込み客に VIS の実力を直感的に理解させやすくなる
- report.md は説明補助・後送資料として位置づけ、本体ではない
