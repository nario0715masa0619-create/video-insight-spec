...
> - そのため、本ドキュメントの記述はローカルでの実装実態（先行実装済みのダッシュボード機能）を基準に同期しています。
...
| **7** | プロダクト拡張 | 🔄 実装中 | **【ローカル実装完了】** StreamlitによるWebダッシュボードが先行構築済み。<br>現在、公開 `main` ブランチへの移行・マージの最終段階。 |
...
### 今後のロードマップ
- **公開 main へのマージ**: ローカルで検証済みの `streamlit_app/` ディレクトリを公開 GitHub の main ブランチに統合（最終マージ作業中）。
