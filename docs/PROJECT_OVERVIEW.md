...
本システムは、ダッシュボードでの可視化やレポートの自動生成に留まらず、生成されるデータ資産自体が**「AI（ChatGPTやGemini等）がそのまま解釈可能な構造化データベース」**として機能するよう設計されています。

### 構造化データ資産の役割分担
1. **`insight_spec_{id}.json` 【最終正本・主たるAI活用データ】**
   - 音声認識とOCRから抽出した知識要素に、Gemini LLM による高度な分類ラベル（ビジネステーマ・難易度・購買ファネルステージ）を付与し、さらに競合・時系列推移をマッピングした最終正本です。AIに食わせるだけで、極めて精度の高いインサイト（勝ちパターン抽出や新企画提案）を引き出せます。
2. **`Mk2_Sidecar_{id}.db` (SQLite) 【補助データ・現状制約あり】**
   - 知識要素とスライドテキストの出現時間を記録するエビデンスDBですが、現行の実データ上の仕様として、ミリ秒時間情報（`start_ms`/`end_ms`）は一律で `(0, 0)` のプレースホルダー値として記録されています。そのため、「動画の何分何秒で話されているか」という秒単位の厳密な検索・質問には現段階では対応しておりません。

### ロードマップにおけるデータ価値強化
- **タイムスタンプの精緻化**: 今後、音声認識のセグメント情報から正確な開始秒・終了秒を取得し、Sidecar DB に反映することで、AI から動画内の特定時間へのピンポイントジャンプや秒単位の解説検索を可能にします。
- **アナリティクス実データの統合**: 視聴維持カーブやCTRの実データを JSON 側に統合することで、AI が「離脱ポイントとコンテンツの因果関係」まで自律的に分析できる環境を目指します。
